...
void search(struct list_node * list, char * key,
            struct list_node ** previous, struct list_node ** current) {
    *previous = list; 
    *current = list->next;
...
struct list_node_2 * create_2(struct list_node *l) {
    struct list_node_2 * aux;
    // Create header node
    aux = (struct list_node_2 *) malloc (sizeof (struct list_node_2));
    if (aux != NULL) {
        aux->p_list_node = l; // início da lista principal
        aux->next = NULL;
    }
    return aux;
}
...
struct list_node_2 * clean_2(struct list_node_2 * list){
    while (list != NULL) {
        struct list_node_2 * temp_ptr = list;
        list = list->next;
        free (temp_ptr);
    }
    return NULL;
...
void search_2(struct list_node_2 * list, int key, struct list_node_2 ** previous, struct list_node_2 ** current){
   *previous = list; 
    *current = list->next;
    while ((*current) != NULL && (*current)->p_list_node->per.age < key) {
        *previous = *current;
        *current = (*current)->next;
    }
}
...
struct list_node_2 * node, * previous, * useless;
    node = (struct list_node_2 *) malloc(sizeof (struct list_node_2));
    if (node != NULL) {
        search_2(list, l->per.age, &previous, &useless);
        node->p_list_node = l;
        node->next = previous->next;
        previous->next = node;
    }
...
void preenche_2 (struct list_node_2 * list, struct list_node * l){
    struct list_node * aux = l->next; /* Skips header */
    while (aux) {
        insert_2 (list, aux);
        aux = aux->next;
    }
...
void print_2(struct list_node_2 * list){
    struct list_node_2 * aux = list->next; /* Skips header */
    while (aux) {
        print_person(aux->p_list_node->per);
        aux = aux->next;
    }
}